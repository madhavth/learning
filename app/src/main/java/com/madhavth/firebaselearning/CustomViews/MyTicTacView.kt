...
class MyTicTacView(context: Context, attributeSet: AttributeSet) : View(context, attributeSet) {
    ...
    private lateinit var mapCentre: Map<Points,Points>
    ...
    fun toggleOpponent() {
        ...
        Snackbar.make(
            this, if (botEnabled) "playing against bot" else "playing against human",
...
//            right.toFloat(), bottom.toFloat(),paintBG)
        ...
        canvas?.drawLine(
            ticTacLayout.right / 3f, ticTacLayout.top.toFloat(),
            ticTacLayout.right / 3f, ticTacLayout.bottom.toFloat(), paintLine
        )
        canvas?.drawLine(
            2 * ticTacLayout.right / 3f, ticTacLayout.top.toFloat(),
        ...
        canvas?.drawLine(
            ticTacLayout.left.toFloat(),
            2 * ticTacLayout.bottom / 3f,
            ticTacLayout.right.toFloat(),
            2 * ticTacLayout.bottom / 3f,
            paintLine
        )
        ...
        if (winner != null) {
            ...
            if (winner == TURN.PLAYER2)
                paintWin.color = Color.BLUE
            else
                paintWin.color = Color.YELLOW

            canvas?.drawLine(minXY.x, minXY.y, maxXY.x, maxXY.y, paintWin)

            Snackbar.make(
                this, if (botEnabled && winner == TURN.PLAYER2)
                    "winner is bot"
        ...
        if(botEnabled && turn == TURN.PLAYER2)
            drawBotMove()
    }


    private fun getMapCentre(): Map<Points,Points>
    {
        val mapCentre = mutableMapOf<Points,Points>()
        for(i in 0..2)
        {
            for(j in 0..2)
            {
                Timber.d("mapCentre $left,$top,$right,$bottom")
                val centreX = (i+1) *(right-left)/3 -(right-left)/6
                val centreY = (i+1) * (bottom-top)/3 -(bottom-top)/6
                mapCentre[Points(i.toFloat(),j.toFloat())] = Points(centreX.toFloat(),centreY.toFloat())
                Timber.d("mapCentre $i,$j -> $centreX, $centreY")
            }
        }
        return mapCentre
    }

    private fun BotMove():Points {
        val points = Points(0f,0f)
        //todo: calculate the best possible move
        //check mapDrawInfos for available spot i.e places to move
        //random move for now
        ...
        for(i in 0..2) {
            for(j in 0..2)
            {
                ...
                Timber.d("i and j are $i, $j")

                if(!mapDrawInfos.keys.contains(Points(i.toFloat(),j.toFloat())))
                    return Points(i.toFloat(),j.toFloat())
            }
        }
        return Points(0f,0f)
    }

    private fun drawBotMove()
    {
        val botMove = BotMove()

        val getCentre = this.mapCentre[botMove]

        val drawInfo = DrawInfo(getCentre!!, TURN.PLAYER2)
        turn = if(turn == TURN.PLAYER1) TURN.PLAYER2 else TURN.PLAYER1
        this.mapDrawInfos[botMove] = drawInfo
